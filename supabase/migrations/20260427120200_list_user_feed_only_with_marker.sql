-- list_user_feed_enriched_cursor: p_only_with_marker — только посты с marker_id (вкладка «события/маркеры»).
-- Взаимоисключающе с p_exclude_with_marker: на клиенте не передаём оба сразу.

drop function if exists public.list_user_feed_enriched_cursor(uuid, int, timestamptz, uuid, uuid, boolean, boolean);
...
comment on function public.list_user_feed_enriched_cursor(
  ...
  'Profile keyset: cluster / only_without_cluster / exclude_with_marker / only_with_marker.';
